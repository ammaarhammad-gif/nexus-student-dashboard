...
def render_animated_progress_bar(percentage: float, color: str = "#38BDF8", height_px: int = 10, label: str = ""):
    ...
    if label:
        label_html = (
            f'<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; font-size: 0.84rem;">'
            f'<span style="font-weight: 600; color: var(--nexus-text-sub);">{label}</span>'
            f'<span style="font-weight: 800; color: {color}; font-family: \'Outfit\', sans-serif;">{rounded_pct}%</span>'
            f'</div>'
        )

    html = (
        f'<div class="nexus-progress-wrapper" style="width: 100%; margin: 6px 0;">'
        f'{label_html}'
        f'<div style="width: 100%; height: {height_px}px; background: rgba(255,255,255,0.08); border-radius: {height_px}px; overflow: hidden; position: relative; box-shadow: inset 0 1px 3px rgba(0,0,0,0.3);">'
        f'<div class="nexus-smooth-bar" style="'
        f'width: {rounded_pct}%; '
        f'height: 100%; '
        f'background: linear-gradient(90deg, {color} 0%, #818CF8 100%); '
        f'border-radius: {height_px}px; '
        f'transition: width 0.9s cubic-bezier(0.34, 1.56, 0.64, 1); '
        f'box-shadow: 0 0 12px {color}66; '
        f'position: relative;'
        f'">'
        f'<div style="position: absolute; right: 0; top: 0; bottom: 0; width: 8px; background: rgba(255,255,255,0.6); filter: blur(2px); border-radius: {height_px}px;"></div>'
        f'</div>'
        f'</div>'
        f'</div>'
    )
