...
def render_breadcrumbs(crumbs: list):
    """
    Renders a sleek responsive breadcrumb trail with Nexus Blue accents.
    crumbs format: [("🏠 Dashboard", "🏠 Dashboard"), ("📚 Syllabus", "📚 Syllabus Manager"), ("Mathematics", None)]
    """
    if not crumbs:
        return
    ...
    for idx, item in enumerate(crumbs):
        ...
        if is_last:
            parts.append(f"<span style='color: var(--nexus-text-title); font-weight: 700;'>{label}</span>")
        else:
            parts.append(f"<span style='color: var(--nexus-text-sub); font-weight: 500;'>{label}</span>")
            
    html = " <span style='color: #64748B; margin: 0 6px;'>›</span> ".join(parts)
    st.markdown(f"""
        <div style="font-size: 0.85rem; margin-bottom: 12px; display: flex; align-items: center; flex-wrap: wrap;">
            {html}
        </div>
    """, unsafe_allow_html=True)


def render_empty_state(icon: str, title: str, message: str, action_button_text: str = None, action_key: str = None) -> bool:
    """Renders a clean glassmorphic empty state box with optional action button."""
    st.markdown(f"""
        <div class="readiness-container" style="text-align: center; padding: 36px 20px; margin: 16px 0;">
            ...
            <p style="font-size: 0.9rem; color: var(--nexus-text-sub); max-width: 480px; margin: 0 auto 16px auto; line-height: 1.5;">
                {message}
            </p>
        </div>
    """, unsafe_allow_html=True)
    if action_button_text:
        ...
        with col2:
            return st.button(action_button_text, key=action_key or f"empty_act_{title}", use_container_width=True, type="primary")
    return False


def render_section_header(badge_icon: str, badge_text: str, title: str, subtitle: str = ""):
    """Renders a consistent, futuristic academic section header."""
    st.markdown(f"""
        <div style="margin-bottom: 18px;">
            <div style="display: inline-flex; align-items: center; gap: 6px; background: rgba(56, 189, 248, 0.12); border: 1px solid rgba(56, 189, 248, 0.35); color: #38BDF8; font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; padding: 4px 12px; border-radius: 20px; margin-bottom: 8px;">
                <span>{badge_icon}</span> <span>{badge_text}</span>
            </div>
